...
def emeval(pos, depth, isplayer):
    ...
    if isplayer:
        ...
        for i in gameState.list_legal_moves(pos):
            ev = min(ev, emeval(MOVES[i](pos, False), depth-1, False))

        return ev
    else:
        ev = 0
        h = gameState.free_squares(pos)
        for i in h:
            ev += (0.9 / len(h)) * emeval(gameState.add_specific_tile(pos, 2, i), depth-1, True)
            ev += (0.1 / len(h)) * emeval(gameState.add_specific_tile(pos, 4, i), depth-1, True)
...
def search(pos, depth, legal):
    ...
    for obj in resultant:
        evaluation.append(emeval(obj, math.ceil(ndepth * (1 - p/16)), False))
