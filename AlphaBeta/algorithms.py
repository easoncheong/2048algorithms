...
def evaluation(pos):
    ev = 0
    ev += algoBase.dynamic_monotonicity_base(pos)
    ev += 100 * algoBase.static_monotonicity_base(pos)
    ev += 2 * algoBase.smoothness_base(pos)

    return ev

def alphabeta(pos, depth, alpha, beta, isplayer):
    if depth == 0:
        return evaluation(pos)
    if gameState.list_legal_moves(pos) == [] and isplayer == True:
        return evaluation(DEAD_POS)
    if isplayer:
        ev = math.inf
        for i in gameState.list_legal_moves(pos):
            ev = min(ev, alphabeta(MOVES[i](pos, False), depth-1, alpha, beta, False))
            beta = min(beta, ev)
            if alpha >= beta:
                break
        return ev
        ...
        ev = -math.inf
        for i in gameState.free_squares(pos):
            ev = max(ev, alphabeta(gameState.add_specific_tile(pos, 2, i), depth-1, alpha, beta, True))
            ev = max(ev, alphabeta(gameState.add_specific_tile(pos, 4, i), depth-1, alpha, beta, True))
            alpha = max(beta, ev)
            if alpha >= beta:
                break
        return ev

def search(pos, depth, legal):
    resultant = []
    evaluation = []
    p = len(gameState.free_squares(pos))
    ndepth = (depth + 1)/2
    for i in legal:
        resultant.append(MOVES[i](pos, False))
    for obj in resultant:
        evaluation.append(alphabeta(obj, math.ceil(ndepth * (1 - p/16)), -math.inf, math.inf, False))

    minEval = math.inf
    selectedMove = -1
    for i in range(len(legal)):
        if evaluation[i] < minEval:
            minEval = evaluation[i]
            selectedMove = legal[i]
    return selectedMove, min(evaluation)
